(() => {
  ...
  function paint(o,target=ctx, camera=state.camera){if(o.type==='text'){target.fillStyle=o.color;target.font=`${o.size*camera.zoom}px Inter, sans-serif`;target.fillText(o.text,o.x*camera.zoom+camera.x,o.y*camera.zoom+camera.y);return}if(o.type==='shape'){const a=screenFor(o.start,camera),b=screenFor(o.end,camera);target.beginPath();if(o.shape==='line'){target.moveTo(a.x,a.y);target.lineTo(b.x,b.y)}else if(o.shape==='rectangle')target.rect(Math.min(a.x,b.x),Math.min(a.y,b.y),Math.abs(b.x-a.x),Math.abs(b.y-a.y));else target.ellipse((a.x+b.x)/2,(a.y+b.y)/2,Math.abs(b.x-a.x)/2,Math.abs(b.y-a.y)/2,0,0,Math.PI*2);target.strokeStyle=o.color;target.lineWidth=o.size*camera.zoom;target.stroke();return}if(o.points.length<2)return;target.beginPath();let p=o.points[0];target.moveTo(p.x*camera.zoom+camera.x,p.y*camera.zoom+camera.y);for(let i=1;i<o.points.length;i++){p=o.points[i];target.lineTo(p.x*camera.zoom+camera.x,p.y*camera.zoom+camera.y)}target.strokeStyle=o.color;target.lineWidth=o.size*camera.zoom*(o.tool==='brush'?1.75:1);target.lineCap='round';target.lineJoin='round';target.globalAlpha=o.tool==='highlighter'?.24:o.tool==='brush'?.55:1;target.stroke();target.globalAlpha=1}
